...
from desktop_paths import InstanceLock, profile_path, resource_root


VERSION = '0.2.0'
INSPIRATION_URL = 'https://github.com/Droski1/PiShock-Unofficial-Documentation'
BG, CARD, INK, MUTED = '#f3f6fa', '#ffffff', '#152439', '#52637a'
NAVY, ACCENT, PALE, LINE = '#142b40', '#087d79', '#e2f4ef', '#dce4ed'
...
class BridgeApp(tk.Tk):
    def __init__(self, *, demo=False, first_run=False):
        ...
        self.install_text = tk.StringVar(value='The bundled add-on supports official firmware 1.4.3 (API 87.1).')
    ...
    def _styles(self):
        ...
        style.configure('TLabel', background=CARD, foreground=INK, font=('Segoe UI', 10))
        style.configure('TButton', padding=(15, 10), font=('Segoe UI Semibold', 10),
                        background='#eaf0f6', foreground=INK, borderwidth=0)
        style.map('TButton', background=[('active', '#dde7f0'), ('disabled', '#eef1f5')],
                  foreground=[('disabled', '#8c99aa')])
        style.configure('Primary.TButton', background=ACCENT, foreground='white')
        style.map('Primary.TButton', background=[('active', '#056965'), ('disabled', '#bdcfcf')],
                  foreground=[('disabled', '#f7f9fa')])
        style.configure('Stop.TButton', background='#ac3443', foreground='white')
        style.map('Stop.TButton', background=[('active', '#8d2634'), ('disabled', '#e3c6cc')])
        style.configure('TCombobox', padding=7, font=('Segoe UI', 10), fieldbackground='white')
        style.configure('TCheckbutton', background=CARD, foreground=INK, padding=5)
    ...
    def _label(self, parent, text='', *, size=10, color=INK, bold=False, bg=CARD, **kw):
        return tk.Label(parent, text=text, bg=bg, fg=color, anchor='w', justify='left',
                        font=('Segoe UI Semibold' if bold else 'Segoe UI', size), **kw)

    def _shell(self):
        self.grid_columnconfigure(1, weight=1)
        self.grid_rowconfigure(0, weight=1)
        sidebar = tk.Frame(self, bg=NAVY, width=208)
        sidebar.grid(row=0, column=0, sticky='nsew')
        sidebar.grid_propagate(False)
        mark = tk.Canvas(sidebar, width=46, height=46, bg=NAVY, highlightthickness=0)
        mark.pack(anchor='w', padx=24, pady=(30, 14))
        mark.create_rectangle(2, 2, 44, 44, fill='#24bbb0', outline='')
        mark.create_line(12, 30, 12, 16, 31, 16, 31, 30, fill=NAVY, width=4)
        ...
        self._label(sidebar, 'FLIPPER + USB', size=9, color='#9fc4d1', bg=NAVY).pack(anchor='w', padx=24, pady=(10, 35))
        self.nav = {}
        for name, text in [('connection', 'Connection'), ('setup', 'Set up devices'), ('help', 'Help & about')]:
            button = tk.Button(sidebar, text=text, anchor='w', padx=18, pady=12,
                               ...
                               bg=NAVY, fg='#cedfe8', activebackground='#21425a', activeforeground='white',
        ...
        self._label(sidebar, 'Desktop app  ' + VERSION + '\nWindows • private local setup',
                    size=9, color='#9eb6c8', bg=NAVY).pack(side='bottom', anchor='w', padx=24, pady=25)
        ...
        self._label(header, 'DEMO · no device or network access' if self.demo else 'Original hub needed only for setup',
                    size=9, color='#7850a1' if self.demo else MUTED, bg=BG).pack(side='right')
        self.container = tk.Frame(workspace, bg=BG)
        ...
        for name in ('connection', 'setup', 'help'):
            host = ScrollPage(self.container)
    ...
    def _connection_page(self):
        ...
        for index, (title, caption) in enumerate([('PiShock', 'Website controls'), ('This computer', 'Internet connection'),
            ...
            tile = tk.Frame(route, bg='#e7edf4')
            tile.grid(row=0, column=index, sticky='nsew', padx=(0, 7 if index < 3 else 0))
            self._label(tile, title, bold=True, bg='#e7edf4').pack(anchor='w', padx=13, pady=(12, 3))
            self._label(tile, caption, size=9, color=MUTED, bg='#e7edf4').pack(anchor='w', padx=13, pady=(0, 12))
    ...
    def show_page(self, name):
        ...
        for key, button in self.nav.items():
            button.configure(bg='#23475d' if key == name else NAVY,
                             fg='white' if key == name else '#cedfe8')
    ...
    def open_guide(self):
        ...
        text = tk.Text(window, wrap='word', padx=24, pady=22, bg=CARD, fg=INK,
...
def self_test():
    ...
    if not serial or not device_backend or not device_transport:
        return 1
    for name in ('pishock_usb_radio-official-1.4.3.fap', 'docs/USER_GUIDE.md', 'LICENSE', 'NOTICE.md'):
